...
char canonb[MAX_TTY_LINE_LENGTH];
...
{
	tp->t_un.t_tc.t_intrc = (char) CINTR;
	tp->t_un.t_tc.t_quitc = (char) CQUIT;
	tp->t_un.t_tc.t_startc = (char) CSTART;
	tp->t_un.t_tc.t_stopc = (char) CSTOP;
	tp->t_un.t_tc.t_eofc = (char) CEOT;
	tp->t_un.t_tc.t_brkc = (char) CBRK;
	tp->t_erase = CERASE;
	tp->t_kill = CKILL;
}
...
loop:
	...
	while ((c=getc(&tp->t_rawq)) >= 0) {
		if ((tp->t_flags&(RAW|CBREAK))==0) {
			...
			if (c==tp->t_kill)
				goto loop;
			if (c==tp->t_un.t_tc.t_eofc)
				continue;
		}
		*bp++ = c;
		if (bp>=canonb+MAX_TTY_LINE_LENGTH)
			break;
	}
	bp1 = &canonb[2];
	...
	if (tp->t_state&TBLOCK && tp->t_rawq.c_cc < TTYHOG/5) {
		if (putc(tp->t_un.t_tc.t_startc, &tp->t_outq)==0) {
			tp->t_state &= ~TBLOCK;
			ttstart(tp);
		}
		tp->t_char = 0;
...
void ttyrend(struct tty *tp, char *pb, char *pe)
{
	...
	if (tp->t_flags&RAW) {
		...
		if (tp->t_chan) {
			/* HG: multiplexor stuff no longer present */
			/* sdata(tp->t_chan); */
		} else {
			wakeup((caddr_t)&tp->t_rawq);
		}
...
void ttyinput(int c, struct tty *tp)
{
	...
	if ((t_flags&RAW)==0) {
		...
		if (tp->t_state&TTSTOP) {
			if (c==tp->t_un.t_tc.t_startc) {
				tp->t_state &= ~TTSTOP;
				ttstart(tp);
				return;
			}
			if (c==tp->t_un.t_tc.t_stopc)
				return;
			tp->t_state &= ~TTSTOP;
			ttstart(tp);
		} else {
		...
		if (c==tp->t_un.t_tc.t_quitc || c==tp->t_un.t_tc.t_intrc) {
			flushtty(tp);
			c = (c==tp->t_un.t_tc.t_intrc) ? SIGINT:SIGQUIT;
			if (tp->t_chan) {
				/* HG: multiplexor stuff no longer present */
	...
	if (t_flags&(RAW|CBREAK)||(c=='\n'||
	                           c==tp->t_un.t_tc.t_eofc||
	                           c==tp->t_un.t_tc.t_brkc)) {
...
register x;
	...
	if (x >= TTYHOG/2) {
		if (putc(tp->t_un.t_tc.t_stopc, &tp->t_outq)==0) {
			tp->t_state |= TBLOCK;
			tp->t_char++;
			ttstart(tp);
		}
