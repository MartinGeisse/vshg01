...
/*
 * Define this macro if EOS32 is to be run on the simulator.
 */
#define RUN_ON_SIMULATOR

#ifdef RUN_ON_SIMULATOR

#define OUTPUT_TO_LOGFILE	TRUE
#define OUTPUT_TO_CONSOLE	FALSE
#define OUTPUT_TO_TERMINAL	FALSE

#else /* RUN_ON_SIMULATOR */

#define OUTPUT_TO_LOGFILE	FALSE
#define OUTPUT_TO_CONSOLE	TRUE
#define OUTPUT_TO_TERMINAL	FALSE

#endif /* RUN_ON_SIMULATOR */

#define K	1024
#define M	(K * K)

#define MEM_MAX	(128 * K)	/* maximum total memory size in frames */
//#define MEM_USE	(1 * K)		/* restricted memory usage in frames */
#define MEM_USE	(512)		/* restricted memory usage in frames */
#define MAXMEM	128		/* max core per process in frames */
...
#define	MAX_TTY_LINE_LENGTH	256		/* max size of typewriter line */
...
#define PAGE_NUMBER	((unsigned)~PAGE_OFFSET)		/* page number mask */
